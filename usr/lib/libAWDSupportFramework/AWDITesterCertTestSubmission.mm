...
- (void)dealloc
{
  ...
  v3.receiver = self;
  v3.super_class = AWDITesterCertTestSubmission;
...
- (id)description
{
  v3.receiver = self;
  v3.super_class = AWDITesterCertTestSubmission;
  return [MEMORY[0x29EDBA0F8] stringWithFormat:@"%@ %@", -[AWDITesterCertTestSubmission description](&v3, sel_description), -[AWDITesterCertTestSubmission dictionaryRepresentation](self, "dictionaryRepresentation")];
}
...
- (void)mergeFrom:(id)from
{
  ...
  if (*(from + 4))
  {
    [(AWDITesterCertTestSubmission *)self setTestName:?];
  }

  if (*(from + 2))
  {
    [(AWDITesterCertTestSubmission *)self setAccessoryName:?];
  }

  if (*(from + 3))
  {

    [(AWDITesterCertTestSubmission *)self setAccessoryType:?];
  }
}
