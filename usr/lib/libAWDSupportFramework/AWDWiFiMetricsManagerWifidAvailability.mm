...
- (void)setHasWifidBootTimeDiff:(BOOL)diff;
- (void)writeTo:(id)to;
@end
...
LABEL_7:
    [dictionary setObject:objc_msgSend(MEMORY[0x29EDBA070] forKey:{"numberWithDouble:", self->_wifidBootTimeDiff), @"wifidBootTimeDiff"}];
    if ((*&self->_has & 4) == 0)
    {
      return dictionary;
    }
...
LABEL_3:
  ...
  {
LABEL_4:
    [dictionary setObject:objc_msgSend(MEMORY[0x29EDBA070] forKey:{"numberWithUnsignedInt:", self->_pid), @"pid"}];
  }
...
LABEL_16:
    ...
    if ((*&self->_has & 2) != 0)
    {
      if ((*(equal + 28) & 2) == 0 || self->_wifidBootTimeDiff != *(equal + 2))
      {
        goto LABEL_16;
      }
    }
    ...
    if ((*&self->_has & 4) != 0)
    {
      if ((*(equal + 28) & 4) == 0 || self->_pid != *(equal + 6))
      {
        goto LABEL_16;
      }
...
LABEL_7:
  ...
  self->_wifidBootTimeDiff = *(from + 2);
  *&self->_has |= 2u;
  if ((*(from + 28) & 4) != 0)
  {
    goto LABEL_7;
