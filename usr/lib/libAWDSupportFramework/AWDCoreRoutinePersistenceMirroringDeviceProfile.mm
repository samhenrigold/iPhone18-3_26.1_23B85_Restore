...
{
  v3.receiver = self;
  v3.super_class = AWDCoreRoutinePersistenceMirroringDeviceProfile;
  return [MEMORY[0x29EDBA0F8] stringWithFormat:@"%@ %@", -[AWDCoreRoutinePersistenceMirroringDeviceProfile description](&v3, sel_description), -[AWDCoreRoutinePersistenceMirroringDeviceProfile dictionaryRepresentation](self, "dictionaryRepresentation")];
}
...
LABEL_13:
  *(result + 9) = self->_visitsAssociatedToDevice;
  *(result + 40) |= 0x40u;
  has = self->_has;
  if ((has & 8) == 0)
  {
...
LABEL_14:
  *(result + 6) = self->_placesAssociatedToDevice;
  *(result + 40) |= 8u;
  has = self->_has;
  if ((has & 0x20) == 0)
  {
...
LABEL_15:
  *(result + 8) = self->_transitionsAssociatedToDevice;
  *(result + 40) |= 0x20u;
  if ((*&self->_has & 0x10) == 0)
  {
    return result;
...
LABEL_8:
  *(result + 7) = self->_timeIntervalSinceLastRecordAssociation;
  *(result + 40) |= 0x10u;
  return result;
}
