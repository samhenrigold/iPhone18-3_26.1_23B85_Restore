...
LABEL_3:
  ...
  {
LABEL_4:
    [dictionary setObject:objc_msgSend(MEMORY[0x29EDBA070] forKey:{"numberWithInt:", self->_code), @"code"}];
  }
...
LABEL_16:
    ...
    if ((*&self->_has & 4) != 0)
    {
      if ((*(equal + 24) & 4) == 0 || self->_reason != *(equal + 5))
      {
        goto LABEL_16;
      }
    }
    ...
    if ((*&self->_has & 2) != 0)
    {
      if ((*(equal + 24) & 2) == 0 || self->_code != *(equal + 4))
      {
        goto LABEL_16;
      }
