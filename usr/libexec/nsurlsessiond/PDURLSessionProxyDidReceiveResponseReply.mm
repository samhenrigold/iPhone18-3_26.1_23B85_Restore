...
- (BOOL)isEqual:(id)equal
{
  ...
  if (*&self->_has)
  {
    if ((*(equalCopy + 24) & 1) != 0 && self->_disposition == *(equalCopy + 2))
    {
      v6 = 1;
      goto LABEL_9;
    }
...
- (id)description
{
  ...
  dictionaryRepresentation = [(PDURLSessionProxyDidReceiveResponseReply *)self dictionaryRepresentation];
...
- (int)StringAsDisposition:(id)disposition
{
  ...
  if ([dispositionCopy isEqualToString:@"Cancel"])
  {
    v4 = 0;
  }

  else if ([dispositionCopy isEqualToString:@"Allow"])
  {
    v4 = 1;
  }

  else if ([dispositionCopy isEqualToString:@"BecomeDownload"])
  {
    v4 = 2;
  }
