...
{
  v7.receiver = self;
  v7.super_class = SDAuthenticationPairingRequest;
  v3 = [(SDAuthenticationPairingRequest *)&v7 description];
  dictionaryRepresentation = [(SDAuthenticationPairingRequest *)self dictionaryRepresentation];
...
LABEL_6:
  ...
  return v5 ^ [(NSData *)self->_longTermKey hash];
}
...
- (void)mergeFrom:(id)from
{
  ...
  if (*(fromCopy + 2))
  {
    [(SDAuthenticationPairingRequest *)self setSessionID:?];
    fromCopy = v6;
  }

  if (*(fromCopy + 1))
  {
    [(SDAuthenticationPairingRequest *)self setLongTermKey:?];
    fromCopy = v6;
  }
}
