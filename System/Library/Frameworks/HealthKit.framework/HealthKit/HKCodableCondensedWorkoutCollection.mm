...
- (id)description
{
  ...
  dictionaryRepresentation = [(HKCodableCondensedWorkoutCollection *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  if ([(NSMutableArray *)self->_workouts count])
  {
    ...
    [dictionary setObject:v4 forKey:@"workouts"];
  }
...
- (void)copyTo:(id)to
{
  ...
  if ([(HKCodableCondensedWorkoutCollection *)self workoutsCount])
  {
    ...
    workoutsCount = [(HKCodableCondensedWorkoutCollection *)self workoutsCount];
    if (workoutsCount)
    {
      v5 = workoutsCount;
      for (i = 0; i != v5; ++i)
  ...
  if (v5)
  {
    ...
    do
    {
      ...
      do
      {
        ...
        [(HKCodableCondensedWorkoutCollection *)self addWorkouts:*(*(&v9 + 1) + 8 * v8++), v9];
      }
