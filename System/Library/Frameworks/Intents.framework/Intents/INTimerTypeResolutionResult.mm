...
+ (id)confirmationRequiredWithTimerTypeToConfirm:(int64_t)confirm;
+ (id)successWithResolvedTimerType:(int64_t)type;
...
+ (id)confirmationRequiredWithTimerTypeToConfirm:(int64_t)confirm
{
  if (!confirm)
  {
    v7 = [MEMORY[0x1E695DF30] exceptionWithName:*MEMORY[0x1E695D940] reason:@"Confirmation of INTimerTypeUnknown is not supported." userInfo:0];
...
+ (id)successWithResolvedTimerType:(int64_t)type
{
  ...
  {
    NSLog(@"Success resolution with INTimerTypeUnknown will be reformed to notRequired.", a2);
...
- (id)_intentSlotValueForObject:(id)object slotDescription:(id)description
{
  ...
  if ([descriptionCopy valueType] == 191)
  {
