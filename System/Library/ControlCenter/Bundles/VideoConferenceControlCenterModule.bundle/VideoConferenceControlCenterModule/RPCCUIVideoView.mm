...
- (RPCCUIVideoView)init
{
  ...
  if (v2)
  {
    ...
    v2->_currentRotation = &off_297D0;

    v3->_mirrorType = 0;
    v3->_orientation = 0;
    portType = v3->_portType;
    v3->_portType = &stru_29390;

    deviceType = v3->_deviceType;
    v3->_deviceType = &off_297D0;

    v3->_intrinsic = xmmword_20D70;
...
LABEL_23:
  ...
  else if ([(__CFString *)v12 containsString:@"PortTypeBack"])
  {
    ...
    if ([(NSNumber *)self->_currentRotation isEqualToNumber:&off_297E8]|| [(NSNumber *)self->_currentRotation isEqualToNumber:&off_29800])
    {
      layer = [(RPCCUIVideoView *)self layer];
      v24 = layer;
      if (layer)
...
- (void)applyTransformForExternalCamera
{
  ...
  if ([(NSNumber *)self->_currentRotation isEqualToNumber:&off_297E8])
  {
  ...
  else if ([(NSNumber *)self->_currentRotation isEqualToNumber:&off_29818])
  {
  ...
  {
    v10 = [(NSNumber *)self->_currentRotation isEqualToNumber:&off_29800];
...
- (void)applyTransformForPortrait
{
  ...
  else if ([(NSNumber *)self->_currentRotation isEqualToNumber:&off_29800])
  {
    memset(&v25, 0, sizeof(v25));
