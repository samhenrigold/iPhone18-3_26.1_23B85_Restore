...
- (NWStatisticsTCPSource)initWithManager:(id)manager local:(const sockaddr *)local remote:(const sockaddr *)remote
{
  ...
  managerCopy = manager;
  v20.receiver = self;
  v20.super_class = NWStatisticsTCPSource;
  v9 = [(NWStatisticsSource *)&v20 initWithManager:managerCopy source:0 provider:2];
  if (!v9)
  {
    goto LABEL_5;
  }
...
LABEL_5:
  ...
  if (os_log_type_enabled(v13, OS_LOG_TYPE_INFO))
  {
    ...
    *buf = 134219266;
    ...
    v31 = 2096;
    remoteCopy = remote;
    _os_log_impl(&dword_25BA3A000, v13, OS_LOG_TYPE_INFO, "Manager %p: %s TCP source for local address %{network:sockaddr}.*P, remote address %{network:sockaddr}.*P", buf, 0x36u);
  }
  ...
  if (([managerCopy mgrflags] & 0x80) != 0)
  {
    ...
    NStatMgrTraceF(managerCopy, "%s Manager %p: %s TCP source for local address %N, remote address %N", "[NWStatisticsTCPSource initWithManager:local:remote:]", managerCopy, v17, local, remote);
  }
