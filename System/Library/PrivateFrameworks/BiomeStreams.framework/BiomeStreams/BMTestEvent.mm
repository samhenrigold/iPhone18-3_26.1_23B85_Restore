...
- (BMTestEvent)initWithCoder:(id)coder;
- (BMTestEvent)initWithContent:(unsigned int)content dataVersion:(unsigned int)version;
- (BMTestEvent)initWithData:(id)data dataVersion:(unsigned int)version;
...
+ (id)eventWithData:(id)data dataVersion:(unsigned int)version
{
  v4 = *&version;
  dataCopy = data;
  v6 = [[BMTestEvent alloc] initWithData:dataCopy dataVersion:v4];
...
- (void)encodeWithCoder:(id)coder
{
  ...
  [coderCopy encodeInt32:p_data->content forKey:@"content"];
}
...
- (BMTestEvent)initWithCoder:(id)coder
{
  ...
  if (-[BMTestEvent checkAndReportDecodingFailureIfNeededForuint32_t:key:coder:errorDomain:errorCode:](self, "checkAndReportDecodingFailureIfNeededForuint32_t:key:coder:errorDomain:errorCode:", v7, @"dataVersion", coderCopy, v6, -1) || (v8 = [coderCopy decodeInt32ForKey:@"content"], -[BMTestEvent checkAndReportDecodingFailureIfNeededForuint32_t:key:coder:errorDomain:errorCode:](self, "checkAndReportDecodingFailureIfNeededForuint32_t:key:coder:errorDomain:errorCode:", v8, @"content", coderCopy, v6, -1)))
  {
    selfCopy = 0;
  }
