...
LABEL_26:
    ...
    sub_1000015F4("[StreamingFileWriter _fileInterfaceResumptionStateForPath:data:fileInterfaceClassName:error:]", 170, @"SZExtractorErrorDomain", 1, 0, 0, @"Unknown file interface type %lu for %@", v39, v26);
  }
  ...
  else
  {
    ...
    sub_1000015F4("[StreamingFileWriter _fileInterfaceResumptionStateForPath:data:fileInterfaceClassName:error:]", 155, @"SZExtractorErrorDomain", 1, v24, 0, @"Failed to decode resumption state for %@", v30, v20);
  }
  v40 = ;
...
+ (id)fileWriterForPath:(id)path withOpenFlags:(int)flags mode:(unsigned __int16)mode quarantineInfo:(void *)info useFSCompression:(BOOL)compression performCachedWrites:(BOOL)writes expectedSize:(int64_t)size asyncTrackingGroup:(id)self0 errorDelegate:(id)self1 error:(id *)self2
{
  ...
  v27 = 0;
  if (qword_10002F170 != -1)
  {
    dispatch_once(&qword_10002F170, &stru_100028730);
  }
