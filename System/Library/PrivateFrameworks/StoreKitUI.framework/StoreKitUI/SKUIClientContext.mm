...
- (SKUIClientContext)initWithConfigurationDictionary:(id)dictionary;
...
+ (id)defaultContext
{
  ...
  v9 = [mainBundle pathForResource:@"SKUIStoreConfigurations" ofType:@"plist"];
...
{
  blockCopy = block;
  uRLBag = [(SKUIClientContext *)self URLBag];
...
- (id)localizedStringForKey:(id)key inTable:(id)table
{
  ...
  v14 = [(SKUILocalizedStringDictionary *)localizedStrings localizedStringForKey:keyCopy inTable:tableCopy];
...
+ (id)localizedStringForKey:(id)key inBundles:(id)bundles inTable:(id)table
{
  ...
  v13 = [[SKUILocalizedStringDictionary alloc] initWithLocaleName:localeIdentifier bundles:bundlesCopy];
  v14 = [(SKUILocalizedStringDictionary *)v13 localizedStringForKey:keyCopy inTable:tableCopy];
...
- (NSArray)navigationHistory
{
  _navigationHistory = [(SKUIClientContext *)self _navigationHistory];
...
- (void)clientInterface:(id)interface presentDialog:(id)dialog
{
  ...
  if (dialogCopy)
  {
    ...
    [(SKUIStoreDialogController *)dialogController presentDialog:dialogCopy];
  }
}

- (id)scriptInterfaceForClientInterface:(id)interface
{
  _applicationController = [(SKUIClientContext *)self _applicationController];
...
- (void)customizePurchase:(id)purchase
{
  ...
  {
    referrerApplicationName = [(SKUIURL *)purchaseReferrerURL referrerApplicationName];
    [purchaseCopy setValue:referrerApplicationName forDownloadProperty:*MEMORY[0x277D6A030]];
    referrerURLString = [(SKUIURL *)self->_purchaseReferrerURL referrerURLString];
...
- (void)customizeSoftwarePurchase:(id)purchase
{
  ...
  {
    referrerApplicationName = [(SKUIURL *)purchaseReferrerURL referrerApplicationName];
    referrerApplicationName2 = [(SKUIURL *)self->_purchaseReferrerURL referrerApplicationName];
...
- (void)loadValueForConfigurationKey:(id)key completionBlock:(id)block
{
  keyCopy = key;
  blockCopy = block;
  v8 = [(SKUIClientContext *)self valueForConfigurationKey:keyCopy];
  if (v8)
  {
    if (blockCopy)
    {
...
- (SSVPlatformContext)platformContext
{
  existingBagDictionary = [(SKUIURLBag *)self->_urlBag existingBagDictionary];
  ...
  {
    uRLBag = [(SKUIClientContext *)self URLBag];
    [uRLBag loadWithCompletionBlock:&__block_literal_global_62];
...
- (id)existingBagValueForKey:(id)key
{
  urlBag = self->_urlBag;
  keyCopy = key;
  existingBagDictionary = [(SKUIURLBag *)urlBag existingBagDictionary];
...
- (int64_t)tabBarStyleForWidth:(double)width
{
  ...
  return *&SKUICompactThreshold_threshold < width;
}
...
+ (NSString)_cachePath
{
  ...
  if (firstObject)
  {
    ...
    firstObject = [v6 stringByAppendingPathComponent:@"SKUIClientContext"];
  }
...
+ (id)_configurationDictionaryWithBagDictionary:(id)dictionary
{
  ...
  if (objc_opt_isKindOfClass())
  {
    ...
    v14 = [dictionaryCopy objectForKey:@"isSponsoredAdsEnabled"];
    if (v14)
    {
      [dictionary setObject:v14 forKey:@"isSponsoredAdsEnabled"];
    }
  }
...
- (void)_setValue:(id)value forConfigurationKey:(id)key
{
  ...
  [defaultCenter postNotificationName:@"SKUIClientContextConfigurationDidChangeNotification" object:self];
}
