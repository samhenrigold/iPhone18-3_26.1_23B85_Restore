...
- (BMOasisAnalyticsGazeEnrollment)initWithPayload:(id)payload;
...
- (BOOL)isEqual:(id)equal
{
  ...
  if (objc_opt_isKindOfClass())
  {
    ...
    {
      payload3 = [(BMOasisAnalyticsGazeEnrollment *)self payload];
      payload4 = [v5 payload];
...
- (void)writeTo:(id)to
{
  if (self->_payload)
  {
    ...
    [(BMOasisAnalyticsGazeEnrollmentPayload *)self->_payload writeTo:toCopy];
...
- (id)initByReadFrom:(id)from
{
  fromCopy = from;
  v20.receiver = self;
  v20.super_class = BMOasisAnalyticsGazeEnrollment;
...
LABEL_16:
      ...
      if ((v14 >> 3) == 1)
      {
        ...
        v15 = [[BMOasisAnalyticsGazeEnrollmentPayload alloc] initByReadFrom:fromCopy];
        if (!v15)
        {
          goto LABEL_25;
        }
...
- (NSString)description
{
  ...
  payload = [(BMOasisAnalyticsGazeEnrollment *)self payload];
  v5 = [v3 initWithFormat:@"BMOasisAnalyticsGazeEnrollment with payload: %@", payload];
...
- (BMOasisAnalyticsGazeEnrollment)initWithPayload:(id)payload
{
  payloadCopy = payload;
  v8.receiver = self;
  v8.super_class = BMOasisAnalyticsGazeEnrollment;
...
+ (id)eventWithData:(id)data dataVersion:(unsigned int)version
{
  ...
  else
  {
    ...
    v8 = [[BMOasisAnalyticsGazeEnrollment alloc] initByReadFrom:v7];
    v4 = v8;
    if (v8)
    {
      v8[4] = 0;
