...
+ (id)protoFields;
- (BMSleepScheduleState)initWithJSONDictionary:(id)dictionary error:(id *)error;
- (BMSleepScheduleState)initWithState:(int)state changeReason:(int)reason;
...
- (BOOL)isEqual:(id)equal
{
  ...
  {
    v5 = equalCopy;
    state = [(BMSleepScheduleState *)self state];
    if (state == [v5 state])
    {
      changeReason = [(BMSleepScheduleState *)self changeReason];
      v8 = changeReason == [v5 changeReason];
    }
    ...
    else
    {
      ...
      if ((objc_opt_isKindOfClass() & 1) == 0)
      {
        if (error)
        {
          ...
          v20 = [objc_alloc(MEMORY[0x1E696AEC0]) initWithFormat:@"Unexpected type %@ for element of %@, expecting NSNumber (corresponding to enum value), or NSString (string version of enum)", objc_opt_class(), @"changeReason"];
          v23 = v20;
...
LABEL_16:
      ...
      if ((v14 >> 3) == 1)
      {
        ...
        if (([fromCopy hasError] & 1) != 0 || v24 > 5)
        {
LABEL_46:
          LODWORD(v24) = 0;
        }
      ...
      else
      {
        ...
        if (([fromCopy hasError] & 1) != 0 || (LODWORD(v24) = v17, v17 > 6))
        {
LABEL_42:
          LODWORD(v24) = 0;
        }
