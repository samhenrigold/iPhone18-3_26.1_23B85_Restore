@interface GEOArrivalParameters
- (GEOArrivalParameters)init;
- (NSMutableArray)arrivalMapRegions;
- (void)_readArrivalMapRegions;
@end

@implementation GEOArrivalParameters

- (NSMutableArray)arrivalMapRegions
{
  [(GEOArrivalParameters *)self _readArrivalMapRegions];
  arrivalMapRegions = self->_arrivalMapRegions;

  return arrivalMapRegions;
}

- (void)_readArrivalMapRegions
{
  if (self)
  {
    os_unfair_lock_lock((self + 48));
    if ((*(self + 60) & 8) == 0)
    {
      v2 = *(self + 8);
      if (v2)
      {
        v3 = v2;
        objc_sync_enter(v3);
        GEOArrivalParametersReadSpecified(self, *(self + 8), &_readArrivalMapRegions_tags);
        objc_sync_exit(v3);
      }
    }

    os_unfair_lock_unlock((self + 48));
  }
}

- (GEOArrivalParameters)init
{
  v6.receiver = self;
  v6.super_class = GEOArrivalParameters;
  v2 = [(GEOArrivalParameters *)&v6 init];
  v3 = v2;
  if (v2)
  {
    v2->_readerLock._os_unfair_lock_opaque = 0;
    v4 = v2;
  }

  return v3;
}

@end