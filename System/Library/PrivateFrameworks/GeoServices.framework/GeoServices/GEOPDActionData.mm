@interface GEOPDActionData
- (GEOPDActionData)init;
- (void)_readActionLinks;
- (void)_readActionTitles;
@end

@implementation GEOPDActionData

- (GEOPDActionData)init
{
  v6.receiver = self;
  v6.super_class = GEOPDActionData;
  v2 = [(GEOPDActionData *)&v6 init];
  v3 = v2;
  if (v2)
  {
    v2->_readerLock._os_unfair_lock_opaque = 0;
    v4 = v2;
  }

  return v3;
}

- (void)_readActionLinks
{
  os_unfair_lock_lock((self + 80));
  if ((*(self + 84) & 2) == 0)
  {
    v2 = *(self + 8);
    if (v2)
    {
      v3 = v2;
      objc_sync_enter(v3);
      GEOPDActionDataReadSpecified(self, *(self + 8), &_readActionLinks_tags);
      objc_sync_exit(v3);
    }
  }

  os_unfair_lock_unlock((self + 80));
}

- (void)_readActionTitles
{
  os_unfair_lock_lock((self + 80));
  if ((*(self + 84) & 8) == 0)
  {
    v2 = *(self + 8);
    if (v2)
    {
      v3 = v2;
      objc_sync_enter(v3);
      GEOPDActionDataReadSpecified(self, *(self + 8), &_readActionTitles_tags);
      objc_sync_exit(v3);
    }
  }

  os_unfair_lock_unlock((self + 80));
}

@end