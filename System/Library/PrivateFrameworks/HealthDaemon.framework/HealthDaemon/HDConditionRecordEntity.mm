...
+ (id)insertDataObject:(id)object withProvenance:(id)provenance inDatabase:(id)database persistentID:(id)d error:(id *)error
{
  ...
  if (([v15 isEqual:objc_opt_class()] & 1) == 0)
  {
    ...
    [currentHandler handleFailureInMethod:a2 object:self file:@"HDConditionRecordEntity.m" lineNumber:69 description:{@"Subclasses must override %s", "+[HDConditionRecordEntity insertDataObject:withProvenance:inDatabase:persistentID:error:]"}];
  }
...
+ (id)entityEncoderForProfile:(id)profile transaction:(id)transaction purpose:(int64_t)purpose encodingOptions:(id)options authorizationFilter:(id)filter
{
  ...
  profileCopy = profile;
  v15 = [(_HDMedicalRecordEntityEncoder *)[_HDConditionRecordEntityEncoder alloc] initWithHealthEntityClass:objc_opt_class() profile:profileCopy transaction:transactionCopy purpose:purpose encodingOptions:optionsCopy authorizationFilter:filterCopy];
