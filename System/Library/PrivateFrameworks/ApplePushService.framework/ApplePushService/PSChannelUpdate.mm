...
{
  statusCopy = status;
  if ([statusCopy isEqualToString:@"CHANNEL_ID_DOES_NOT_EXIST"])
  {
    v4 = 0;
  }
...
- (id)description
{
  ...
  dictionaryRepresentation = [(PSChannelUpdate *)self dictionaryRepresentation];
...
LABEL_13:
  ...
  if (*&self->_has)
  {
    if ((*(equalCopy + 28) & 1) == 0 || self->_checkpoint != *(equalCopy + 1))
    {
      goto LABEL_13;
    }
