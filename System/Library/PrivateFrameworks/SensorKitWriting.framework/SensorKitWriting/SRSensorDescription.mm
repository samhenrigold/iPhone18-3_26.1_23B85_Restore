...
- (NSString)localizedAdditionalSampleDataNote
{
  ...
  if (!result)
  {
    v4 = [MEMORY[0x277CCA8D8] bundleForClass:NSClassFromString(&cfstr_Srsensorreader.isa)];
    if (v4)
    {
      result = [v4 localizedStringForKey:self->_additionalSampleDataNoteKey value:&stru_2877002B0 table:0];
...
- (NSString)sampleClass
{
  ...
  else
  {
    ...
    {
      *v7 = 0;
      _os_log_fault_impl(&dword_26561F000, v6, OS_LOG_TYPE_FAULT, "Found a linked before version but no legacy sample class", v7, 2u);
    }
