...
- (FTTextNormalizationRequest)initWithFlatbuffData:(id)data root:(const TextNormalizationRequest *)root verify:(BOOL)verify;
- (NSString)language;
- (NSString)session_id;
- (NSString)speech_id;
- (NSString)text;
...
- (FTTextNormalizationRequest)initWithFlatbuffData:(id)data root:(const TextNormalizationRequest *)root verify:(BOOL)verify
{
  ...
  dataCopy = data;
  v25.receiver = self;
  v25.super_class = FTTextNormalizationRequest;
  v10 = [(FTTextNormalizationRequest *)&v25 init];
  if (!v10)
  {
    goto LABEL_13;
  }
  ...
  v10->_root = root;
  if (verifyCopy && ((v12 = [(NSData *)v10->_data bytes], v13 = [(NSData *)v10->_data length], root = v10->_root, root >= v12) ? (v15 = root > v12 + v13) : (v15 = 1), v15 || (v16 = [(NSData *)v10->_data bytes], v17 = [(NSData *)v10->_data length], v21[0] = v16, v21[1] = v17, v22 = xmmword_233005E20, v23 = 0, v24 = 1, (v18 = v10->_root) != 0) && !siri::speech::schema_fb::StartPronGuessRequest::Verify(v18, v21)))
  {
LABEL_14:
    v19 = 0;
  }
...
- (Offset<siri::speech::schema_fb::TextNormalizationRequest>)addObjectToBuffer:(void *)buffer
{
  speech_id = [(FTTextNormalizationRequest *)self speech_id];
  v6 = speech_id;
  if (!speech_id)
  {
  ...
  session_id = [(FTTextNormalizationRequest *)self session_id];
  v11 = session_id;
  if (!session_id)
  {
  ...
  language = [(FTTextNormalizationRequest *)self language];
  v16 = language;
  if (!language)
  {
  ...
  text = [(FTTextNormalizationRequest *)self text];
  v21 = text;
  if (!text)
  {
    text = &stru_284834138;
  ...
  nbest_variants_max = [(FTTextNormalizationRequest *)self nbest_variants_max];
  *(buffer + 70) = 1;
  v25 = *(buffer + 10);
  ...
  apple::aiml::flatbuffers2::FlatBufferBuilder::AddElement<unsigned int>(buffer, 12, nbest_variants_max, 0);
...
- (id)flatbuffData
{
  ...
  v10 = 256;
  v11 = 0;
  apple::aiml::flatbuffers2::FlatBufferBuilder::Finish(&v3, [(FTTextNormalizationRequest *)self addObjectToBuffer:&v3], 0, 0);
  operator new();
}
