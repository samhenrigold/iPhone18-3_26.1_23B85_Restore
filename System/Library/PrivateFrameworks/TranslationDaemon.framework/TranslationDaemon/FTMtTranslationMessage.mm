...
- (FTMtTranslationMessage)initWithFlatbuffData:(id)data root:(const MtTranslationMessage *)root verify:(BOOL)verify;
- (FTTranslationRequest)session_messageAsFTTranslationRequest;
- (FTTranslationResponse)session_messageAsFTTranslationResponse;
- (Offset<siri::speech::qss_fb::MtTranslationMessage>)addObjectToBuffer:(void *)buffer;
- (id)flatbuffData;
...
- (FTMtTranslationMessage)initWithFlatbuffData:(id)data root:(const MtTranslationMessage *)root verify:(BOOL)verify
{
  ...
  dataCopy = data;
  v29.receiver = self;
  v29.super_class = FTMtTranslationMessage;
  v10 = [(FTMtTranslationMessage *)&v29 init];
  v11 = v10;
  if (v10)
  {
    ...
    if (verifyCopy)
    {
      ...
      if (v16 || (v17 = [(NSData *)v10->_data bytes], v18 = [(NSData *)v10->_data length], v24 = v17, v25 = v18, v26 = xmmword_233005E20, v27 = 0, LOBYTE(v28) = 1, (v19 = v10->_root) != 0) && !siri::speech::qss_fb::MtTranslationMessage::Verify(v19, &v24))
      {
LABEL_15:
        v22 = 0;
        goto LABEL_16;
...
- (FLTBFBufferAccessor)session_message
{
  session_message_type = [(FTMtTranslationMessage *)self session_message_type];
  if (session_message_type == 2)
  {
    session_messageAsFTTranslationResponse = [(FTMtTranslationMessage *)self session_messageAsFTTranslationResponse];
  }

  else if (session_message_type == 1)
  {
    session_messageAsFTTranslationResponse = [(FTMtTranslationMessage *)self session_messageAsFTTranslationRequest];
  }

  else
  {
    session_messageAsFTTranslationResponse = 0;
  }

  return session_messageAsFTTranslationResponse;
}
...
- (Offset<siri::speech::qss_fb::MtTranslationMessage>)addObjectToBuffer:(void *)buffer
{
  ...
  if ([(FTMtTranslationMessage *)self session_message_type]== 1)
  {
    session_messageAsFTTranslationRequest = [(FTMtTranslationMessage *)self session_messageAsFTTranslationRequest];
    v7 = [session_messageAsFTTranslationRequest addObjectToBuffer:buffer];
  }
  ...
  {
    session_messageAsFTTranslationResponse = [(FTMtTranslationMessage *)self session_messageAsFTTranslationResponse];
    v9 = [session_messageAsFTTranslationResponse addObjectToBuffer:buffer];
...
- (id)flatbuffData
{
  ...
  v10 = 256;
  v11 = 0;
  apple::aiml::flatbuffers2::FlatBufferBuilder::Finish(&v3, [(FTMtTranslationMessage *)self addObjectToBuffer:&v3], 0, 0);
  operator new();
}
