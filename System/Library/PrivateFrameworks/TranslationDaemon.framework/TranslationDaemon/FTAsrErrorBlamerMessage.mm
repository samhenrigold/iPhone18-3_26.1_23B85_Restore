...
- (FTAsrErrorBlamerMessage)initWithFlatbuffData:(id)data root:(const AsrErrorBlamerMessage *)root verify:(BOOL)verify;
- (FTErrorBlamerRequest)session_messageAsFTErrorBlamerRequest;
- (FTErrorBlamerResponse)session_messageAsFTErrorBlamerResponse;
- (Offset<siri::speech::qss_fb::AsrErrorBlamerMessage>)addObjectToBuffer:(void *)buffer;
- (id)flatbuffData;
...
- (FTAsrErrorBlamerMessage)initWithFlatbuffData:(id)data root:(const AsrErrorBlamerMessage *)root verify:(BOOL)verify
{
  ...
  if (v10)
  {
    ...
    if (verifyCopy)
    {
      ...
      if (v16 || (v17 = [(NSData *)v10->_data bytes], v18 = [(NSData *)v10->_data length], v24 = v17, v25 = v18, v26 = xmmword_233005E20, v27 = 0, LOBYTE(v28) = 1, (v19 = v10->_root) != 0) && !siri::speech::qss_fb::AsrErrorBlamerMessage::Verify(v19, &v24))
      {
LABEL_15:
        v22 = 0;
        goto LABEL_16;
...
- (FLTBFBufferAccessor)session_message
{
  session_message_type = [(FTAsrErrorBlamerMessage *)self session_message_type];
  if (session_message_type == 2)
  {
    session_messageAsFTErrorBlamerResponse = [(FTAsrErrorBlamerMessage *)self session_messageAsFTErrorBlamerResponse];
  }

  else if (session_message_type == 1)
  {
    session_messageAsFTErrorBlamerResponse = [(FTAsrErrorBlamerMessage *)self session_messageAsFTErrorBlamerRequest];
  }

  else
  {
    session_messageAsFTErrorBlamerResponse = 0;
  }

  return session_messageAsFTErrorBlamerResponse;
}
...
- (Offset<siri::speech::qss_fb::AsrErrorBlamerMessage>)addObjectToBuffer:(void *)buffer
{
  ...
  if ([(FTAsrErrorBlamerMessage *)self session_message_type]== 1)
  {
    session_messageAsFTErrorBlamerRequest = [(FTAsrErrorBlamerMessage *)self session_messageAsFTErrorBlamerRequest];
    v7 = [session_messageAsFTErrorBlamerRequest addObjectToBuffer:buffer];
  }
  ...
  {
    session_messageAsFTErrorBlamerResponse = [(FTAsrErrorBlamerMessage *)self session_messageAsFTErrorBlamerResponse];
    v9 = [session_messageAsFTErrorBlamerResponse addObjectToBuffer:buffer];
