...
- (SAWaitInfoWithPortLabel)initWithKCDataWaitInfo:(const stackshot_thread_waitinfo_v2 *)info
{
  ...
  {
    *v6 = 0;
    _os_log_error_impl(&dword_1E0E2F000, v4, OS_LOG_TYPE_ERROR, "SAWaitInfoWithPortLabel init with port info", v6, 2u);
  }

  *__error() = v3;
  _SASetCrashLogMessage(3104, "SAWaitInfoWithPortLabel init with port info");
  result = _os_crash();
  __break(1u);
  return result;
}
