...
- (MBServiceCache)initWithPath:(id)path domainManager:(id)manager
{
  ...
  if (v6)
  {
    ...
    if (qword_100421948 != -1)
    {
      dispatch_once(&qword_100421948, &stru_1003C0658);
    }
...
LABEL_21:
    ...
    {
      sqlite3_trace_v2(self->_db, 1u, sub_100192C10, 0);
    }
...
LABEL_20:
  ...
  if (!self->_db)
  {
    [+[NSAssertionHandler currentHandler](NSAssertionHandler handleFailureInMethod:"handleFailureInMethod:object:file:lineNumber:description:" object:a2 file:self lineNumber:@"MBServiceCache.m" description:432, @"Missing handle for open cache db"];
  }
...
- (void)close
{
  ...
  if (openCount >= 1)
  {
    if (openCount == 1)
    {
      if (!self->_db)
      {
        [+[NSAssertionHandler currentHandler](NSAssertionHandler handleFailureInMethod:"handleFailureInMethod:object:file:lineNumber:description:" object:a2 file:self lineNumber:@"MBServiceCache.m" description:502, @"Missing handle for open cache db"];
      }
...
- (id)snapshotsForBackupUDID:(id)d
{
  ...
  [v6 reset];
  [v5 sortUsingComparator:&stru_1003C06A0];
  return v5;
}
