...
- (void)setCryptoKey:(id)key
{
  ...
  MEMORY[0x2821F9670](allValues, sel_makeObjectsPerformSelector_withObject_);
}
    ...
    while (1)
    {
      ...
      if (v12 > 0x636D)
      {
        if (v12 == 25454)
        {
          ...
          if (v13 && *v10 == v15[0])
          {
            v14 = [objc_alloc(MEMORY[0x277CBEA90]) initWithBytes:bufferCopy + 8 length:(v9 - 4)];
        ...
        {
          self->mEncryptedDocumentUuid = [objc_alloc(MEMORY[0x277CBEA90]) initWithBytes:bufferCopy + 8 length:(v9 - 4)];
        }
      }
      ...
      {
        self->mPassphraseVerifier = [objc_alloc(MEMORY[0x277CBEA90]) initWithBytes:bufferCopy + 8 length:(v9 - 4)];
      }
