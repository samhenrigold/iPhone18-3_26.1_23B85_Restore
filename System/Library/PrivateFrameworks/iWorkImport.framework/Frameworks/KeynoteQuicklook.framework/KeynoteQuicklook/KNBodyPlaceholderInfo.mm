...
- (KNBodyPlaceholderInfo)initWithKNPlaceholderInfo:(id)info
{
  ...
  v17.receiver = self;
  v17.super_class = KNBodyPlaceholderInfo;
  v12 = [(KNBodyPlaceholderInfo *)&v17 initWithContext:context2 geometry:geometry style:shapeStyle pathSource:pathSource wpStorage:v7];

  if (v12)
  {
    -[KNBodyPlaceholderInfo setLocked:](v12, "setLocked:", [infoCopy isLocked]);
    hyperlinkURL = [infoCopy hyperlinkURL];
    [(KNBodyPlaceholderInfo *)v12 setHyperlinkURL:hyperlinkURL];

    comment = [infoCopy comment];
    [(KNBodyPlaceholderInfo *)v12 setComment:comment];

    accessibilityDescription = [infoCopy accessibilityDescription];
    [(KNBodyPlaceholderInfo *)v12 setAccessibilityDescription:accessibilityDescription];

    -[KNBodyPlaceholderInfo setAspectRatioLocked:](v12, "setAspectRatioLocked:", [infoCopy aspectRatioLocked]);
  }
...
- (id)copyToInstantiateTemplatePlaceholder
{
  ...
  copyToInstantiateTemplatePlaceholder = [(KNBodyPlaceholderInfo *)&v8 copyToInstantiateTemplatePlaceholder];
...
- (void)acceptVisitor:(id)visitor
{
  ...
  if (v5)
  {
    [v5 visitKNBodyPlaceholderInfo:{self, &unk_2885462B0}];
  }

  else
  {
    v7.receiver = self;
    v7.super_class = KNBodyPlaceholderInfo;
    [(KNBodyPlaceholderInfo *)&v7 acceptVisitor:visitorCopy, &unk_2885462B0];
  }
}
...
- (id)typeName
{
  ...
  v3 = [v2 localizedStringForKey:@"Body" value:&stru_2884D8E20 table:@"Keynote"];
