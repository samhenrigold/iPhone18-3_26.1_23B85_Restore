...
- (id)description
{
  ...
  dictionaryRepresentation = [(NPKProtoSecureElementGetPairingInfoRequest *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  if (*&self->_has)
  {
    ...
    [dictionary setObject:v4 forKey:@"setAuthRandomIfNotPaired"];
  }
...
- (void)copyTo:(id)to
{
  if (*&self->_has)
  {
    *(to + 8) = self->_setAuthRandomIfNotPaired;
    *(to + 12) |= 1u;
  }
}
...
- (void)mergeFrom:(id)from
{
  if (*(from + 12))
  {
    self->_setAuthRandomIfNotPaired = *(from + 8);
    *&self->_has |= 1u;
  }
}
