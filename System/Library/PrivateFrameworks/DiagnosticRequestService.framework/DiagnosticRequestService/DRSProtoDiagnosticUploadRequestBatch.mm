...
- (void)addUploadRequests:(id)requests;
- (void)copyTo:(id)to;
...
- (void)addUploadRequests:(id)requests
{
  ...
  [(NSMutableArray *)uploadRequests addObject:requestsCopy];
}
...
- (NSString)description
{
  ...
  dictionaryRepresentation = [(DRSProtoDiagnosticUploadRequestBatch *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  if ([(NSMutableArray *)self->_uploadRequests count])
  {
    ...
    [dictionary setObject:v6 forKey:@"upload_requests"];
  }
...
- (void)copyTo:(id)to
{
  ...
  {
    [toCopy clearUploadRequests];
    uploadRequestsCount = [(DRSProtoDiagnosticUploadRequestBatch *)self uploadRequestsCount];
    if (uploadRequestsCount)
    {
      v5 = uploadRequestsCount;
      for (i = 0; i != v5; ++i)
      {
        v7 = [(DRSProtoDiagnosticUploadRequestBatch *)self uploadRequestsAtIndex:i];
        [toCopy addUploadRequests:v7];
      }
    }
  }
}
  ...
  if (v8)
  {
    ...
    do
    {
      for (i = 0; i != v9; ++i)
      {
        ...
        [(DRSProtoDiagnosticUploadRequestBatch *)self addUploadRequests:*(*(&v12 + 1) + 8 * i), v12];
      }
