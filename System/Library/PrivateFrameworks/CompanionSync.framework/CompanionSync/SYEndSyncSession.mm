...
- (id)description
{
  ...
  v8.receiver = self;
  v8.super_class = SYEndSyncSession;
  v4 = [(SYEndSyncSession *)&v8 description];
  dictionaryRepresentation = [(SYEndSyncSession *)self dictionaryRepresentation];
...
- (id)dictionaryRepresentation
{
  ...
  if (*&self->_has)
  {
    v9 = [MEMORY[0x1E696AD98] numberWithBool:self->_rollback];
    [dictionary setObject:v9 forKey:@"rollback"];
  }
...
- (void)mergeFrom:(id)from
{
  ...
  else
  {
    ...
    [(SYEndSyncSession *)self setHeader:?];
  }
...
LABEL_7:
  if (*(fromCopy + 3))
  {
    [(SYEndSyncSession *)self setSessionID:?];
    fromCopy = v9;
  }
  ...
  else
  {
    ...
    error = [(SYEndSyncSession *)self setError:?];
  }
